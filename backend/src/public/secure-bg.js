(function(){
  ...
  function applyBg(url){
    ...
    try {
      if (window.localStorage) {
        if (fit === 'contain') {
          chosen = localStorage.getItem(LS_SRC) || chosen;
        } else {
          chosen = localStorage.getItem(LS_KEY) || chosen;
        }
      }
    } catch(e){}
    if (!chosen) chosen = '/image/cdcNC.png';
  ...
  function applyFieldOffsets(){
    ...
    function valOf(key){ var v = 0; try { if (window.localStorage) v = parseInt(localStorage.getItem(key) || '0', 10) || 0; } catch(e){} return clampOffset(v); }
  ...
  function setMoveMode(on){
    ...
    Object.keys(targets).forEach(function(k){ var el = targets[k].el; if (!el) return; el.style.cursor = moveMode ? 'move' : ''; el.style.outline = ''; });
  ...
  function readOffset(key){ var v = 0; try { if (window.localStorage) v = parseInt(localStorage.getItem(key) || '0', 10) || 0; } catch(e){} return clampOffset(v); }
  function writeOffset(key, v){ try { if (window.localStorage) localStorage.setItem(key, String(clampOffset(v|0))); } catch(e){} }
  var dragging = null;
  ...
  function setMoveBgMode(on){
    ...
    try { if (window.localStorage) { var saved = localStorage.getItem(LS_KEY); if (saved) applyBg(saved); else applyBg(''); } } catch(_){ }
  }
  var draggingBg = null;
  function onBgDown(e){
  ...
  if (cardEl) {
    ...
    cardEl.addEventListener('touchstart', function(ev){ if (!moveBgMode) return; var t = ev.touches && ev.touches[0]; if (!t) return; onBgDown({ preventDefault: function(){ ev.preventDefault(); }, clientX: t.clientX, clientY: t.clientY }); }, { passive: false });
    cardEl.addEventListener('touchmove', function(ev){ if (!draggingBg) return; var t = ev.touches && ev.touches[0]; if (!t) return; onBgMove({ clientX: t.clientX, clientY: t.clientY }); ev.preventDefault(); }, { passive: false });
  ...
  if (fitCoverEl) fitCoverEl.addEventListener('change', function(){ if (fitCoverEl.checked) { try { if (window.localStorage) localStorage.setItem(LS_FIT, 'cover'); } catch(e){} var saved = null; try { if (window.localStorage) saved = localStorage.getItem(LS_KEY); } catch(e){} applyBg(saved || ''); } });
  if (fitContainEl) fitContainEl.addEventListener('change', function(){ if (fitContainEl.checked) { try { if (window.localStorage) localStorage.setItem(LS_FIT, 'contain'); } catch(e){} var saved = null; try { if (window.localStorage) saved = localStorage.getItem(LS_KEY); } catch(e){} applyBg(saved || ''); } });
  if (overlayEl) overlayEl.addEventListener('input', function(){ var val = Math.max(0, Math.min(0.35, parseFloat(overlayEl.value || '0.15'))); try { if (window.localStorage) localStorage.setItem(LS_OVERLAY, String(val)); } catch(e){} var saved = null; try { if (window.localStorage) saved = localStorage.getItem(LS_KEY); } catch(e){} applyBg(saved || ''); });
